#!/bin/bash
# Evaluate Jina v5-omni models on MMEB visdoc benchmark
set -e
...
for i in "${!MODELS[@]}"; do
  ...
  uv run python experiments/eval_jina_mmeb.py \
    ...
    2>&1 | tee "${OUTPUT_DIR}/eval_visdoc.log"

  echo "Done: $NAME / visdoc"
done

echo ""
echo "All Jina visdoc evaluations complete."
