    ...
    with open(data_args.dataset_config, 'r') as yaml_file:
        ...
        if data_args.data_basedir:
            for _, task_config in dataset_config.items():
                for path_key in ['image_dir', 'dataset_path', 'video_frame_basedir']:
                    path_value = task_config.get(path_key)
                    if path_value and not os.path.isabs(path_value):
                        task_config[path_key] = os.path.join(data_args.data_basedir, path_value)
